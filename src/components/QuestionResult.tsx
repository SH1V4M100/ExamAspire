...
const QuestionResult: React.FC<QuestionResultProps> = ({
  ...
  const selectedOptionIds = userAnswer?.selectedOptionIds || [];
  const selectedValue = selectedOptionIds[0];

  const isIntegerType = question.questionType === 'integer';

  let isCorrect = false;
  let userTypedValue: string | undefined = undefined;
  let correctValue: string | undefined = undefined;
  ...
  if (isIntegerType) {
    ...
    if (selectedValue === correctOption?.id) {
      isCorrect = true;
      userTypedValue = correctOption?.text??'blank';
    } else {
      isCorrect = false;
      userTypedValue = selectedValue;
    }
  } else {
    const correctOptionIds = question.options
      .filter((option) => option.isCorrect)
                  ...
                  <div className="text-sm">
                    ...
                    {!isCorrect && correctValue && (
                      <p className="mt-1 text-sm text-blue-600">
                        Correct Answer:{' '}
                        <span className="font-semibold">{correctValue}</span>
                      </p>
                    )}
                  </div>
                </div>
                  ...
                  return (
                    <div key={option.id} className={optionClass}>
                      ...
                      <div className="flex-1">
                        <p
                          className={`text-sm ${
                            isCorrectOption ? 'font-medium text-blue-600' : 'text-gray-700'
                          }`}
                        >
                          {option.text}
                        </p>
